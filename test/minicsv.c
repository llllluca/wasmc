...
static void _minicsv_out_col(char* const col_start, char** const cols, size_t* const cols_count_p,
...
static void _minicsv_parse_line_noquote(const int c, char** const pos_p, char** const col_start_p,
...
static void _minicsv_parse_line_quote(const int c, char** const pos_p, int* const state_p)
{
    if (c == MINICSV_QUOTE) {
        if (*(*pos_p + 1U) == MINICSV_QUOTE) {
            my_memmove(*pos_p, *pos_p + 1U, my_strlen(*pos_p));
        }
        else {
            **pos_p = 0;
            *state_p = 0;
